...
IMAGE_MAX_BYTES = 12 * 1024 * 1024  # base64 раздувает ~×1.33; крупные картинки жрут память
    ...
    async def buildConverstaion(self, messages: list[disnake.Message]) -> list[dict]:
        ...
        for mes in messages:
            if mes.author == self.bot.user:
                ...
                if (attachment is not None and mes is last_message and has_vision
                        and is_image and attachment.size <= IMAGE_MAX_BYTES):
